...
class _AppointmentScreenState extends State<AppointmentScreen>
  ...
  loadMoreUpcomingAppointments() async {
    if (upcomingNextUrl != "null" && !isLoadingMoreUpcoming) {
      ...
      print("ooooooooooo" + Uri.parse("$upcomingNextUrl&user_id=$id").toString() );
      final jsonResponse = jsonDecode(response.body);
      if (response.statusCode == 200 && jsonResponse['status'] == 1) {
        print(jsonResponse);
        upcomingAppointments = UpcomingAppointments.fromJson(jsonResponse);
  ...
  fetchPastAppointments() async {
    ...
    print("ssssssssssssss" + Uri.parse("$SERVER_ADDRESS/api/getuserpastappointment?user_id=$id").toString());
    print(response.request.url);
    final jsonResponse = jsonDecode(response.body);
    if (response.statusCode == 200 && jsonResponse['status'] == 1) {
      print(jsonResponse);
